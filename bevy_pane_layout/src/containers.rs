...
use bevy::{
    ecs::system::ResMut,
    feathers::theme::{ThemeToken, UiTheme},
    ui::Val,
};
...
pub fn setup(mut ui_theme: ResMut<UiTheme>, theme: &Theme) {
    ui_theme.set_color(
        "feathers.pane.header.bg",
        theme.pane.header_background_color.0,
    );
    ...
    ui_theme.set_color(
        "feathers.subpane.body.bg",
        theme.pane.area_background_color.0,
    );
