...
impl WinitWindows {
    ...
    pub fn create_window(
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            if let Some(icon) = load_window_icon_from_env() {
                winit_window_attributes =
                    winit_window_attributes.with_window_icon(Some(icon.clone()));
                winit_window_attributes =
                    winit_window_attributes.with_taskbar_icon(Some(icon));
            }
        }
...
#[cfg(target_os = "windows")]
fn load_window_icon_from_env() -> Option<Icon> {
    candidate_window_icon_paths()
        .into_iter()
        .find_map(load_window_icon_from_path)
}

#[cfg(target_os = "windows")]
fn candidate_window_icon_paths() -> Vec<std::path::PathBuf> {
        ...
        .and_then(|path| path.parent().map(std::path::Path::to_path_buf))
    {
        ...
        candidates.push(exe_dir.join("..").join("..").join("assets").join("logo.png"));
    }
...
#[cfg(target_os = "windows")]
fn load_window_icon_from_path(path: std::path::PathBuf) -> Option<Icon> {
    let image = image::ImageReader::open(path).ok()?.decode().ok()?.into_rgba8();
